...
class RealSenseYOLOCupDetector:
    ...
    def detect_cups(self, color_image):
        ...
        end_time = time.perf_counter()
        print(f"cost time is {end_time - start_time} s")
        cv2.waitKey(1)
        return []
    ...
    def visualize(self, color_image, depth_colormap, detections, positions):
        ...
        """
        print(f"visualize 1")
        if False and detections and positions:
            print(f"visualize 2")
            # 在彩色图像上绘制检测结果
        ...
        print(f"visualize 6")
        # images = np.random.randint(0, 255, (300, 300, 3), dtype=np.uint8)
    ...
    def run(self):
        ...
        try:
            while True:
                # 获取帧
                print(f"before get frames")
                color_image, depth_image, depth_colormap = self.get_frames()
                print(f"after get frames")
                if color_image is None or depth_image is None:
                    continue
                
                print(f"before detect cups")
                detections = None
                # 检测杯子
                detections = self.detect_cups(color_image.copy())
                print(f"after detect cups")
...
if __name__ == "__main__":
    # 初始化检测器
    detector = RealSenseYOLOCupDetector(model_name='yolov8n', conf_threshold=0.5)
